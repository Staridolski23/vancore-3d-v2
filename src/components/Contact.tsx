...
export default function Contact() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setStatus(null);
    try {
      const res = await fetch(`${API_URL}/contact`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, message }) });
      if (!res.ok) throw new Error('failed');
      setStatus('✅ Запитването е изпратено. Ще се свържем с вас скоро.');
      setName('');
      setEmail('');
      setMessage('');
    } catch (err) {
    ...
    <section id='контакт' className='relative py-32'>
      <div className='relative z-10 max-w-7xl mx-auto px-6'>
        <div className='text-center mb-16'>
          ...
          <h2 className='text-3xl md:text-5xl font-black mb-4'>Свържете се с нашия екип</h2>
        </div>
        <div className='grid md:grid-cols-2 gap-10'>
          <form onSubmit={submit} className='glass rounded-3xl p-8 border border-white/5 space-y-5'>
            <div>
              <label className='block text-xs text-vancore-muted mb-2'>Име Фамилия</label>
            ...
            <div>
              <label className='block text-xs text-vancore-muted mb-2'>Опишете накратко проблема...</label>
            ...
            <button type='submit' disabled={loading} className='w-full py-2.5 rounded-lg bg-gradient-to-r from-vancore-bronze to-vancore-gold text-vancore-dark font-semibold disabled:opacity-50'>Изпрати запитване</button>
          ...
          <div className='glass rounded-3xl p-8 border border-white/5 space-y-4 text-sm text-vancore-muted'>
            ...
            <p>Имейл: hello@vancoresys.com</p>
            <p>Телефон: +359 88 000 0000</p>
            <div>
              <h4 className='font-semibold text-vancore-light'>⚡ Директен AI анализ</h4>
              <p>Можете да тествате безплатния анализ директно в секция AI Анализ.</p>
            </div>
          </div>
        </div>
      </div>
