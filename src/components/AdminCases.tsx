...
export default function AdminCases() {
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${statusStyles[c.status] || 'bg-white/5 text-vancore-muted border-white/10'}`}>
      ...
      {selectedCase && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelectedCaseId(null)}>
          <div className="glass rounded-3xl border border-white/10 p-6 max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">Казус: {selectedCase.company}</h3>
            <div className="space-y-2 text-sm text-vancore-muted">
              <p><span className="text-vancore-light">Отрасъл:</span> {selectedCase.industry}</p>
              <p><span className="text-vancore-light">Проблем:</span> {selectedCase.problem}</p>
              <p><span className="text-vancore-light">Статус:</span> {selectedCase.status}</p>
              <p><span className="text-vancore-light">Дата:</span> {selectedCase.date}</p>
              {selectedCase.analysis && (
                <p><span className="text-vancore-light">Анализ:</span> {selectedCase.analysis}</p>
              )}
            </div>
            <div className="mt-4 flex justify-end">
              <button onClick={() => setSelectedCaseId(null)} className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-xs hover:bg-white/10">Затвори</button>
            </div>
          </div>
        </div>
      )}
