...
export default function Methodology() {
  ...
  const title = section?.title || t('methodology.title');
  const subtitle = section?.subtitle || t('methodology.subtitle');
  const titleHtml = title.replace('{highlight}', '<span class="gradient-text">').replace('{/highlight}', '</span>');
    ...
    <section id="методология" className="relative py-32">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-20">
          ...
          <h2 className="text-3xl md:text-5xl font-black mb-4" dangerouslySetInnerHTML={{ __html: titleHtml }} />
              ...
              <div className="space-y-6">
                {pillars.map((item: string, idx: number) => (
                  <div key={idx} className="flex gap-4">
                    ...
                    <p className="text-sm text-vancore-muted">{item}</p>
                  </div>
                ))}
              </div>
            </div>
